...
int main(){
    ...
    while(i<=input_value){
        ...
        int j = 1;
        // here we write value=i because i is the row and in every row-loop it increments by 1 and we have to start our each row by the distance of one, so by storing our value of row in a variable 'val' we can do this and after that column while-loop we will increment i/row by 1 and this will print our pattern
        int val = i;
